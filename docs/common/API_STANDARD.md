# DDUK ERP API Standard

이 문서는 기존 `API_DRAFT.md`를 `common` 도메인의 API 기준 문서로 이동한 것이다.  
프론트엔드, 백엔드, AI 서버가 동시에 작업할 수 있게 핵심 엔드포인트와 공통 규칙을 정리한다.

비고:

- 기존 초안성 설명은 삭제하지 않고 유지한다.
- 최종 확정되지 않은 항목은 문서 안에서 "예상", "초안", "2차 확장"으로 표시한다.
